...
def generate_batch(
    ...
    # Because we pad to same length, we can slice with global L
...
# COCO helpers
# ----------------------------
def load_coco(coco_root: str, split: str) -> Tuple[COCO, str, str]:
...
def report_metrics(Y_true: np.ndarray, Y_pred: np.ndarray, name: str):
    ...
    # Still useful for quick comparisons; for real mAP you'd need per-class scores.
    ap_list = []
...
def main():
    ...
    parser.add_argument("--split", type=str, default="val2017", choices=["val2017", "train2017"])
    ...
    parser.add_argument("--k", type=int, default=30, help="Max number of images to evaluate after slicing")
    parser.add_argument("--shuffle", action="store_true")
    parser.add_argument("--seed", type=int, default=123)

    # NEW: sharding/slicing
    parser.add_argument("--start_idx", type=int, default=0, help="Start index into the (optionally shuffled) img_id list")
    parser.add_argument("--end_idx", type=int, default=-1, help="End index (exclusive). -1 means 'to the end'.")

    # NEW: batching
    parser.add_argument("--batch_size", type=int, default=1, help="Batch size for generation (increase until you hit OOM)")
    ...
    # NEW: jsonl output
    parser.add_argument("--out_jsonl", type=str, default="", help="Write per-image outputs (pred/raw/prompts/gt) to a JSONL file")
    ...
    coco, ann_file, img_dir = load_coco(args.coco_root, args.split)
    ...
    sliced = img_ids[start:end]
    if args.k is not None and args.k > 0:
        sliced = sliced[:args.k]
    subset = sliced
    ...
    for base in range(0, n, args.batch_size):
        ...
        # captions for sanity can also be batched
        captions = None
        if args.sanity_check:
        ...
        for j, (img_id, info, out) in enumerate(zip(batch_img_ids, batch_infos, outs)):
            ...
            # JSONL write (per image)
            writer.write(record)

            # sanity dump (per image)
            if args.sanity_check:
