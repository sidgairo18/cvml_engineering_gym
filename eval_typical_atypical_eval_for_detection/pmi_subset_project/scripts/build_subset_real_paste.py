#!/usr/bin/env python3
# allow running as a plain script without -m or __init__.py
import os, sys
...
def crop_instance(api, img, ann):
    ...
    if box is None: return None, None, None
    x,y,w,h = box
...
def build_one(bg_api, obj_api, pair, per_type=200, out_dir='out', ioa_place=0.4, drop_cover=0.95, occl_bins=(0,0.1,0.3,0.5), save_masks=True):
    a, b = pair['a'], pair['b']
    ...
    for typ in types:
        for idx in tqdm(range(per_type), desc=f"{a}-{b} [{typ}]"):
            ...
            target_h_a = int(np.random.randint(64, 192))
            ...
            target_h_b = int(np.random.randint(64, 192))
            ...
            cv2.imwrite(img_path, out, [int(cv2.IMWRITE_JPEG_QUALITY), 95])

            maskA_path = None; maskB_path = None
            if save_masks:
                maskA_path = os.path.join(mask_dir, out_name.replace('.jpg','_A.png'))
                maskB_path = os.path.join(mask_dir, out_name.replace('.jpg','_B.png'))
                cv2.imwrite(maskA_path, (Ma>0).astype('uint8')*255)
                cv2.imwrite(maskB_path, (Mb>0).astype('uint8')*255)

            manifests[typ].append({
                'file_name': out_name,
                'height': H, 'width': W,
                'mask_paths': {'A': (maskA_path and os.path.relpath(maskA_path, out_dir)),
                               'B': (maskB_path and os.path.relpath(maskB_path, out_dir))},
                'objects': [
                    {'category': a, 'bbox': boxA, 'iscrowd': 0},
                    {'category': b, 'bbox': boxB, 'iscrowd': 0}
                ],
                'pair': {'a':a,'b':b,'type':typ,'relation':relation, 'match_id': pair.get('match_id')}
            })
