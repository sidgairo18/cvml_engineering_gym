...
def main():
    ...
    typical = [dict(a=row['cat_a_name'], b=row['cat_b_name'], type='typical', source='pmi', pmi=float(row['pmi']))
               for _,row in typical_df.iterrows()]

    atypical_df = df.sort_values('pmi', ascending=True).head(args.top_atypical*3)
    atypical = []
