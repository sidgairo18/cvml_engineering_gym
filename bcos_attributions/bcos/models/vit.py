...
class PatchEmbed(nn.Module):                                                                  
    ...
    """                                                                                       
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, conv2d_layer=None):     
        super().__init__()                                                                    
        ...
        self.proj = conv2d_layer(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
...
class DINOLikeSimpleViT(nn.Module):                                                       
    def __init__(                                                                         
        self,                                                                             
        *,                                                                                
        ...
        **kwargs                                                                          
        ...
        self.patch_embed = PatchEmbed(                                                    
                img_size=image_size, patch_size=patch_size, in_chans=channels, embed_dim=embed_dim, conv2d_layer=conv2d_layer)
        self.num_patches = self.patch_embed.num_patches                                         
                                                                                          
        if self.use_cls_token:                                                            
            print("Model uses the cls token!")
            #self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))                   
            self.cls_token = nn.Parameter(torch.randn(1, 1, embed_dim))*100
            #self.cls_token = nn.Parameter(torch.randint(1, 100000, (1, 1, embed_dim)).float())
            self.pos_embed = nn.Parameter(torch.zeros(1, self.num_patches + 1, embed_dim))
        else:                                                                             
            self.pos_embed = nn.Parameter(torch.zeros(1, self.num_patches, embed_dim))    
        # we should also account for dropout and stochastic path dropping as DINO implementation. ignored for now.
        ...
        # only do this for non-Bcos models (this hacky check can be improved)
        if channels > 3:
            pass
...
class BcosDINOHead(nn.Module):                                                                
    def __init__(self, in_dim, out_dim, use_bn=False, norm_last_layer=True, nlayers=3, hidden_dim=2048, bottleneck_dim=256, linear_layer=BcosLinear, norm_layer=NoBias(DetachableLayerNorm), act_layer=nn.Identity):
        super().__init__()                                                                    
...
def vitc_ti_patch1_7(my_configs=CONFIGS):
    #kwargs.setdefault("num_classes", 1_000)
    return SimpleViT(
        image_size=7,
        patch_size=1,
...
def vitc_s_patch1_7(my_configs=CONFIGS):
    #kwargs.setdefault("num_classes", 1_000)
    return SimpleViT(
        image_size=7,
        patch_size=1,
...
def vitc_b_patch1_7(my_configs=CONFIGS):
    #kwargs.setdefault("num_classes", 1_000)
    return SimpleViT(
        image_size=7,
        patch_size=1,
...
def dino_like_vitc_ti_patch1_14(my_configs=CONFIGS):                                      
                                                                                          
    return DINOLikeSimpleViT(                                                             
        image_size=224,                                                                   
        patch_size=32,                                                                    
        depth = 12,                                                                       
        dim=384 // 2,                                                                     
...
def dino_like_vitc_s_patch1_14(my_configs=CONFIGS):                                       
...
def dino_like_vitc_b_patch1_14(my_configs=CONFIGS):                                       
    return DINOLikeSimpleViT(                                                             
        image_size=224,                                                                   
        patch_size=32,                                                                    
        depth = 12,                                                                       
