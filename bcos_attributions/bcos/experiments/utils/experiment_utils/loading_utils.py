...
def get_state_dict_and_training_ckpt_from_save_dir(
    ...
    reload : str
        ...
        "best_any": similar to "best", however will load either ema or non-ema
        depending on which accuracy was the best.
        "best" will load best ema weights if `ema=True` and best non-ema otherwise
        "best_any" == "best" if `ema=False`
        Default: "last"
    ema : bool
...
def _determine_best_epoch_and_ema_status(
    ...
    ema : bool
        Whether to consider using EMA weights.
    reload : str
        The reload type.
    ...
    """
    assert reload in ["best", "best_any"], "Only 'best' or 'best_any' supported!"
    try:
        metrics = Metrics.from_experiment_dir(save_dir)
    except MetricsNotFoundError:
    ...
    # if ema, we need to check that instead (or if best_any is used,
    # we need to determine which accuracy is better)
    if ema:
        if reload == "best":
            warnings.warn(
                "Non-EMA Weights might be better. Consider using reload='best_any'"
            )
        best_epoch_ema, best_acc_ema = metrics.get_best_epoch_and_accuracy_ema()
        # if best_any is used, we need to determine weights to use based on accuracy
        ema = reload == "best" or best_acc_ema > best_acc
        best_epoch = best_epoch_ema if ema else best_epoch
