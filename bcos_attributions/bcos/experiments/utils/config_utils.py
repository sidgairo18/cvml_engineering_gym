...
def flatten_config(config_dict: Dict, prefix: str = "", separator: str = "."):
    """
    This flattens the config dict so that the hyperparameters can be nicely
    logged as strings or numbers. In particular, with W&B.
    This works recursively.
    Thanks, @Amin Parchami for this!
    ...
    prefix : str
        The prefix to use for the keys in the flattened result.
    separator : str
        The separator for parent and child keys.

    Notes
    -----
    If a *value* object has the method `__flatten_config__`, then that will be
    ...
    Dict[str, str | int | float]
        A flattened dictionary with all string keys and all
        values as strings or numbers.
    """
    flattened_dict_result = dict()
    for key, value in config_dict.items():
        new_key = f"{prefix}{separator}{key}" if prefix else key

        if isinstance(value, dict):  # subconfig
            flattened_dict_result.update(
                flatten_config(value, prefix=new_key, separator=separator)
            )
        elif isinstance(value, (str, int, float)):  # one of allowed types
            flattened_dict_result[new_key] = value
        elif hasattr(value, "__flatten_config__"):  # non-dict object's subconfig
            sub_config_dict = value.__flatten_config__()
            flattened_dict_result.update(
                flatten_config(sub_config_dict, prefix=new_key, separator=separator)
            )
        else:  # just use repr if nothing else possible
            flattened_dict_result[new_key] = repr(value)

    return flattened_dict_result
