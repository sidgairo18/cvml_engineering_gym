...
DEFAULT_BATCH_SIZE = 128  # per GPU! * 4 = 1024 effective
...
baseline = {
    f"{name}": update_default(
        dict(
            ...
            optimizer=OptimizerFactory("AdamW", lr=DEFAULT_LR, weight_decay=0.0001),
            use_agc=False,
            trainer=dict(
...
bcos = {
    f"bcos_{name}": update_default(
        dict(
            ...
            optimizer=OptimizerFactory("Adam", lr=DEFAULT_LR),
        )
    )
    for name in SIMPLE_VIT_ARCHS
}
